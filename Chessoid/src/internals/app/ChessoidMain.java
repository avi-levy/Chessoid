...
public class ChessoidMain extends Activity {
	...
	/**
	 * List of references to all tiles on the board. For internal use only, not
	 * to be referenced outside this class.
	 */
	...
	public void moveIcon(View v) {
		if (v instanceof ChessTileView) {
			// TODO: this functionality may need to be decomposed into various
			// smaller methods or use handler classes
			ChessTileView tile = (ChessTileView) v;
			ChessPiece piece = tile.getChessPiece();
			if (null != piece) {
				// if clipboard is null place the piece on the clipboard (select
				// this piece to move next)
				if (null == clipboard) {
					// put piece on the clipboard to be dropped on next tile
					// clicked
					clipboard = tile;
					// TODO: animate the currently selected chess piece
				} else {

					// we're trying to move to a tile occupied by another piece,
					// try to capture
					ChessPiece attacker = clipboard.getChessPiece();
					ChessPiece victim = piece;
					try {
						// doMove() will try a capture if possible
						ChessGame.INSTANCE.processMove(attacker.location,
								victim.location);
				...
				if (null != clipboard) {
					...
					if (queuedPiece != null) {
						...
						try {
							...
							ChessGame.INSTANCE
									.processMove(queuedPiece.location, tile
											.getChessCoords());

							tile.setChessPiece(queuedPiece);
							clipboard.setChessPiece(null); // <-- clear the
							// previous tile
	...
	/**
	 * Create a TableLayout and add it to this Activity as the content view
	 * (setContentView()). Adds 8 rows and columns and sets up the chess board
	 * layout with alternating black and white tiles.
	 */
	public void initTableLayout() {
		...
		for (int i = 8; i >= 1; i--) {
			...
			for (int j = 1; j <= 8; j++) {
				...
				ChessTileView tv = new ChessTileView(this, white ? 0xFFFFFFFF
						: 0x00000000, coords);
				chessTiles.add(tv);
				...
				tr.addView(tv);
				white = !white; // <-- toggle between black and white tile while
				// creating table
			}
	...
	public void initChessBoard() {
		ChessBoard board = ChessGame.INSTANCE.getChessBoard();
		for (ChessTileView tile : chessTiles) {
//			tile.setChessPiece(chessSet.get(tile.getChessCoords()));
			tile.setChessPiece(board.getChessPieceAt(tile.getChessCoords()));
		}		
	}
